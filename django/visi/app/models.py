...
class User(AbstractUser):
    ...
    profilename = models.CharField(
        max_length=100
    )  # Name on profile different from username and full name
...
class Fan(models.Model):
    ...
    eventsattended = models.ManyToManyField(
        "Events", through="Tickets", related_name="fan_eventsattended"
    )
...
#--Model for doorman user--#
class Doorman(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="doorman_user"
    )

    def __str__(self):
        return self.user.profilename

#--Model for artist user--#
class Artist(models.Model):
    ...
    artistimage = models.ImageField(
        upload_to="artistpics/", default="artistpics/default.png", null=True, blank=True
    )
...
class Venue(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="venue_user"
    )
    location = models.ForeignKey(
        "Location", on_delete=models.CASCADE, related_name="venue_location"
    )
    venuecapacity = models.IntegerField()
    venueimage = models.ImageField(
        upload_to="venuepics/", default="venuepics/default.png", null=True, blank=True
    )
...
class Events(models.Model):
    ...
    eventimage = models.ImageField(
        upload_to="eventpics/", default="eventpics/default.png", null=True, blank=True
    )
    EventArtists = models.ManyToManyField(
        Artist, through="EventArtists", related_name="event_artists"
    )
    eventvenue = models.ForeignKey(
    ...
    authorized_doormen = models.ManyToManyField(
        Doorman, related_name="events", blank=True
    )

    def __str__(self):
        return self.eventname

#--Model that connects artist to an event--#
class EventArtists(models.Model):
    artist = models.ForeignKey(
        Artist, on_delete=models.CASCADE, related_name="event_artist"
    )
    event = models.ForeignKey(
        Events, on_delete=models.CASCADE, related_name="event_artist"
    )
